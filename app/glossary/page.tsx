 ...
 * - Uses the same mock data as the detail pages
 */

// ---------- MOCK STORE (same as detail page) ----------
type GlossaryItem = {
  slug: string;
  term: string;
  excerpt: string;
...
function buildGlossaryJsonLd() {
  ...
  const collectionPage = {
    ...
    '@type': 'CollectionPage',
    name: 'Glossary',
    description: 'Definitions and explanations of key terms in global payroll and remote work.',
    url,
    mainEntity: {
      '@type': 'ItemList',
  ...
  return JSON.stringify([webPage, breadcrumbs, collectionPage]);
}
...
export default function GlossaryPage() {
  ...
  const [filteredTerms, setFilteredTerms] = React.useState(Object.values(GLOSSARY_STORE));

  const allTerms = Object.values(GLOSSARY_STORE);
  
  // Get all unique first letters
  const availableLetters = React.useMemo(() => {
    const letters = new Set(allTerms.map(term => term.term[0].toUpperCase()));
  ...
  const displayedTerms = showAll || searchTerm || selectedLetter !== 'ALL' 
    ? filteredTerms 
    : filteredTerms.slice(0, 9);

  const schemaJson = buildGlossaryJsonLd();

  return (
    <>
      {/* JSON-LD */}
      <Script
        id="glossary-index-jsonld"
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6 py-16 md:py-24">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-neutral-600 max-w-3xl mx-auto">
              Understand key terms in global payroll, payments, and remote work. 
              From USD wallets to FX fees, we've got you covered.
            </p>
          </div>
          ...
          <div className="relative max-w-2xl mx-auto mb-8">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 border border-neutral-200 rounded-2xl text-lg focus:outline-none focus:ring-2 focus:ring-[#0E2C1E] focus:border-transparent"
            />
          </div>
          ...
          <div className="mb-8">
            <p className="text-neutral-600 text-center">
              {showAll || searchTerm || selectedLetter !== 'ALL' 
                ? `${filteredTerms.length} ${filteredTerms.length === 1 ? 'term' : 'terms'}`
                : `Showing ${Math.min(9, filteredTerms.length)} of ${filteredTerms.length} terms`}
          ...
          {displayedTerms.length > 0 ? (
            <>
                ...
                {displayedTerms.map((term, index) => (
                  ...
                  >
                    <Link
                      ...
                      className="block p-6 border border-neutral-200 rounded-2xl hover:border-[#0E2C1E] hover:shadow-lg transition-all duration-200 group"
                    >
                      <h3 className="text-xl font-semibold text-neutral-900 mb-3 group-hover:text-[#0E2C1E] transition-colors">
                        {term.term}
                      </h3>
                      <p className="text-neutral-600 leading-relaxed mb-4">
                        {term.excerpt}
                      </p>
                      <div className="flex items-center text-sm text-[#0E2C1E] font-medium">
                        Learn more
              ...
              {/* Show All Button */}
              {!showAll && selectedLetter === 'ALL' && !searchTerm && filteredTerms.length > 9 && (
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-[#0E2C1E] text-white px-8 py-4 font-semibold hover:bg-[#0a1f15] transition-all hover:scale-105"
                  >
                    See All {filteredTerms.length} Terms
            ...
            <div className="text-center py-12">
              ...
              <p className="text-neutral-600">
                Try adjusting your search terms or browse all glossary entries.
              </p>
            </div>
          )}
          ...
          >
            <div className="rounded-3xl bg-[#0E2C1E] text-white p-8 md:p-12">
              <h2 className="text-2xl md:text-3xl font-bold mb-4">
                Ready to build your global team?
              </h2>
              <p className="text-white/80 mb-6 max-w-2xl mx-auto">
                Now that you understand the terms, see how Sigma can help you 
                implement global payroll with USD wallets and transparent pricing.
              </p>
              <Link
                href="/book-demo"
                className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-neutral-900 font-semibold hover:bg-neutral-100 transition-colors"
              >
                Get Started Today
